"""
00_check_ambiente.py - Verifica se o ambiente da Aula 6 esta pronto.

Testa (um item por linha, com OK ou FALHA):
  1. Python
  2. Bibliotecas (haystack + hierarchical/auto-merging, sklearn/umap p/ RAPTOR, langfuse, etc.)
  3. .env e chaves
  4. Corpus + perguntas da Aula 6
  5. OpenSearch + kNN
...
def checar_bibliotecas():
    ...
    libs = [
        ...
        "sklearn",          # RAPTOR (clustering)
        "umap",             # RAPTOR (reducao dimensional)
        "langfuse",
        "ollama",
        "dotenv",
    ]
    for lib in libs:
        try:
            ...
            falha(f"{lib} -> {e}")
    # componentes especificos da Aula 6
    try:
        from haystack.components.preprocessors import HierarchicalDocumentSplitter  # noqa: F401
        from haystack.components.retrievers import AutoMergingRetriever  # noqa: F401
        ok("HierarchicalDocumentSplitter + AutoMergingRetriever (parent-child)")
    except Exception as e:
        falha(f"componentes hierarquicos -> {e}")
...
def checar_corpus():
    print("4) Corpus + perguntas da Aula 6")
    try:
        docs, perguntas = _comum.carregar_corpus()
        ok(f"{len(docs)} documentos | {len(perguntas)} perguntas de teste")
    except Exception as e:
...
def checar_langfuse():
    ...
    if not _comum.langfuse_configurado():
        aviso("chaves do LangFuse ausentes - o chat (05) roda sem tracing. Veja Aula 3.")
        return
...
def main():
    parser = argparse.ArgumentParser(description="Verifica o ambiente da Aula 6.")
    parser.add_argument("--testar-groq", action="store_true")
    args = parser.parse_args()

    print("=" * 60)
    print("  CHECAGEM DE AMBIENTE - MBA RAG & CAG - Aula 6")
    print("=" * 60)
    checar_python()
    checar_bibliotecas()
    checar_env()
